...
1. **在Finally块中清理资源**或者使用try-with-resource语句
...
**内存泄漏**：程序申请了空间，用完之后却没有被GC回收，始终占用着内存，既不能被使用也不能分配给其他程序

##### 1. 堆内存和栈内存溢出的区别

- **栈内存溢出**：对于一台服务器而言，每一个用户请求，都会产生一个线程来处理这个请求，每一个线程对应着一个栈，栈会分配内存，此时如果请求过多，这时候内存不够了，就会发生栈内存溢出
- **栈溢出**：不断的调用方法，不断的压栈，最终超出了栈允许的栈深度，就会发生栈溢出，比如递归操作没有终止，死循环
- **堆内存溢出**：堆中主要存储的是对象。如果不断的new对象则会导致堆中的空间溢出

##### 2. 常见内存泄漏的场景

**1）没有释放的资源连接**，如数据库连接、网络连接等。这种资源的连接只有代码中显示的调用close()才能够关闭资源，如果没有调用close()方法，资源连接是不会被GC垃圾回收器回收的。

**2）单例模式(singleton)**：单例对象在被初始化后将在JVM的整个生命周期中存在（以静态变量的方式），如果单例对象持有外部对象的引用，那么这个外部对象将不能被jvm正常回收，导致内存泄露

**3）修改HashSet中对象的参数值，且参数是计算哈希值的字段（HashMap类似）**
